	...
	<div id="container">
		...
		<div id="content">
			<div id="board">
				...
				<div class="selected">
					<c:if test="${prev}">
						<span>[ <a href="${pageContext.request.contextPath }/board?a=search&num=${startPageNum -1}&kwd=${kwd}&option=${option}">이전</a> ]</span>
					</c:if>
										
						<c:forEach begin="${startPageNum}" end="${endPageNum}" var="num">
							<span>
								<c:if test="${select != num }">
									<a href="${pageContext.request.contextPath }/board?a=search&num=${num}&kwd=${kwd}&option=${option}">${num}</a>
								</c:if>
					...
					<c:if test="${next}">
						<span>[ <a href="${pageContext.request.contextPath }/board?a=search&num=${endPageNum+1}&kwd=${kwd}&option=${option}">다음</a> ]</span>
					</c:if>
				</div>
				</div>
				<!-- pager 추가 -->				
